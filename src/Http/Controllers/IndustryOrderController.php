...
class IndustryOrderController extends Controller
{
    ...
    public function expiredOrders(): Factory|Application|View|\Illuminate\Contracts\Foundation\Application
    {
        $orders = Order::expiredOrders();
        $statistics = OrderStatistic::generateAll();

        return view('seat-industry::orders.expiredOrders', compact('orders', 'statistics'));
    }
    ...
    public function confirmOrder(Order $order, Request $request): RedirectResponse
    {
        ...
        Gate::authorize('seat-industry.modify-order', $order);
    ...
    {
        Gate::authorize('seat-industry.corp_delivery', $order);
        if ($order->corp_id) {
            $order->corp_id = null;
        } else {
            $order->corp_id = auth()->user()->main_character->affiliation->corporation_id;
