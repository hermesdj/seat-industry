<table class="delivery-item-table table table-striped table-hover">
    ...
    @foreach($items as $item)
            ...
            <tr>
                ...
                <td class="d-flex flex-row text-center align-middle" style="min-width: 120px;">
                    @can("seat-industry.same-user",$delivery->user_id)
                        <form action="{{ route("seat-industry.setDeliveryItemState", ['delivery' => $delivery->id, 'item' => $item->id]) }}"
                              method="POST"
                              style="width: 50%"
                        >
                            @csrf
                        ...
                        @if(!$item->completed || auth()->user()->can("seat-industry.admin"))
                            <form action="{{ route("seat-industry.deleteDeliveryItem",['delivery' => $delivery->id, 'item' => $item->id]) }}"
                                  method="POST"
                                  style="width: 50%">
                                @csrf
                                <button type="submit"
                                        class="btn btn-danger btn-sm text-nowrap confirmform ml-1 btn-block"
        ...
        @else
            <tr><td>Order No Longer Exists, cannot display items.</td></tr>
        @endif
    @endforeach
    </tbody>
</table>
